...
10b. **Estabilizar antes de lançar.** **Duas checagens seguidas**, com 2 s entre elas (mais um respiro opcional via `DISCORD_VPN_LAUNCHER_ESTABILIZACAO`, padrão 0) — adaptador do túnel com IPv4 + país `!= BR` — dentro de uma janela de 30 s. Falha isolada zera o contador; estourar a janela é falha de sessão (popup). Uma confirmação única prova que o túnel *chegou* a subir, não que ele está firme — ver 15.12.
...
Agora são duas checagens seguidas (adaptador com IPv4 + país `!= BR`, 2 s entre elas). Uma falha isolada **não** condena a sessão — oscilar logo após a troca de rotas é normal, então o contador zera e a janela de 30 s continua correndo. Não estabilizar dentro dela, sim, é falha: melhor o popup com as duas saídas do que um Discord aberto por um túnel que não existe mais.
...
### 15.13 O que o console mostra nao e o que o log guarda

Dois ajustes vindos do primeiro teste da 1.4.0 na maquina real, um deles reportado por usuarios da 1.3.

**O ruido de retry saiu da tela.** Logo depois de o tunel subir, a primeira consulta de pais falha por DNS ainda em troca e o console imprimia `Tentativa 1: ipinfo.io: Este host nao e conhecido; ...`. E o comportamento normal - a tentativa seguinte responde, e a sessao segue - mas usuarios da 1.3 leram aquilo como erro e reportaram. O detalhe de cada tentativa passou para `Log.Diag`, que escreve **so** no arquivo; a tela recebe uma linha calma e unica (`Aguardando as rotas da VPN assentarem...`) apenas quando a demora passa de 8 s, o que o caso normal (~5 s) nunca alcanca. Mesma regra para as checagens de `EstabilizarTunelAsync`. Tirar do console nao e esconder problema: o `launcher.log` continua com o erro de cada servico, e o que exige acao do usuario ou explica falha real continua em `Warn`/`Error`.

**O respiro fixo da estabilizacao virou zero.** Ele nasceu como 5 s de "deixar o tunel assentar", mas o assentamento ja tinha acontecido: `ConfirmarPaisAsync` so retorna quando uma consulta responde de fato. O que sobrava era espalhar a observacao no tempo para um relay moribundo se revelar - e disso ja cuidam as duas checagens separadas por 2 s. Espera fixa que nao acrescenta sinal e so ping ruim cobrado do usuario, entao o padrao passou a ser 0 e a variavel de ambiente ficou para quem quiser um respiro maior. Vale como regra ao mexer aqui: espera fixa nova precisa de um sinal novo que a justifique.
